...
static volatile ControlStatus g_defaultStatus = {90, 90};
static volatile ControlStatus g_currentStatus = {90, 90};
...
static void can_build_pow_mirror_state(CanFrame *frame, void* user) {
    (void)user;
    CanMessage msg = {0};
    msg.pow_wheel_state.sig_wheel_angle         = g_currentStatus.wheel_angle;
    msg.pow_wheel_state.sig_wheel_position      = g_currentStatus.wheel_position;
...
void fsm_setTargetStatus(const CanMessage *message) {
    g_control.target.wheel_position   = message->dcu_wheel_order.sig_wheel_position;
    g_control.target.wheel_angle      = message->dcu_wheel_order.sig_wheel_angle;
}
