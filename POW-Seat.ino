...
static void can_build_pow_mirror_state(CanFrame *frame, void* user) {
    (void)user;
    CanMessage msg = {0};
    msg.pow_seat_state.sig_seat_angle           = g_currentStatus.seat_angle;
    msg.pow_seat_state.sig_seat_position        = g_currentStatus.seat_position;
    msg.pow_seat_state.sig_seat_front_height    = g_currentStatus.seat_front_height;
    msg.pow_seat_state.sig_seat_rear_height     = g_currentStatus.seat_rear_height;
...
static void can_setup(void){
    ...
    if (can_register_job_dynamic(CH, &jobID_pow_seat_state, can_build_pow_mirror_state, NULL, 20) != CAN_OK)
...
static void sensor_loop() {
    bool raw = (digitalRead(SEAT_PIN) == LOW);
...
void fsm_beginNotReadySequence() {
    // TODO: 전원/버스/센서/모터 초기화, 기준점 복귀 등
    // 예: pwm.begin(), sensor.init(), homing.start() ...
}

// NotReady 단계: 매 프레임 호출, 준비 조건 충족 여부 반환
bool fsm_updateNotReadyAndCheckReady() {
    // TODO: 준비 진행(논블로킹) & 조건 검사
    // 예: if (homing.isDone() && sensor.ok() && comms.ok()) return true;
    CanMessage msg = {0};
...
void fsm_onEnterBusy() {
    // TODO: 목표값으로 제어 시작(논블로킹 루틴 준비)
    // 예: motion.startMove(order.targetValue);
    g_control.active = true;
}

// Busy 단계: 매 프레임 호출, 작업 완료되면 true 반환
bool fsm_updateBusyAndCheckDone() {
    // TODO: 목표치 도달까지 제어(논블로킹)
    // 예: if (motion.update() && motion.atTarget()) return true;
    return false;
}

// 안전 정지/리셋 시 호출
void fsm_stopAllModules() {
    // TODO: 모터/PWM/밸브 등 안전 정지
    // 예: motion.stop(); pwm.setAllOff(); etc.
}
...
void fsm_setTargetStatus(const CanMessage *message) {
    g_control.target.seat_front_height    = message->dcu_seat_order.sig_seat_front_height;
    g_control.target.seat_rear_height     = message->dcu_seat_order.sig_seat_rear_height;
    g_control.target.seat_position        = message->dcu_seat_order.sig_seat_position;
    g_control.target.seat_angle           = message->dcu_seat_order.sig_seat_angle;
}
...
void setup() {
    ...
    delay(100);
    // 통신/버스/모듈 등 공통 초기화
    // TODO: Wire.begin(), pwm.begin(), comms.init() 등
    can_setup();
    relay_setup();
    fsm_setup();
}
